...
eps = 0.05
delm_prop = 40
sentence_min_length, sentence_max_length = 3, 10
...
def next_word(text, N, counts):
    ...
    for w in words:
        if not w in counts[token_seq]:
            # if not w in ['.','!', '?', ',']:
            if '.' in w or u'.' in w or u'.' in w:
            	choices.append((w,100000 * eps))
            else:
...
def preprocess_corpus(filename):
    s = open(filename, 'r').read().decode('utf-8')
    s = s.replace(u'،', ' ')
    s = s.replace(u'«', ' ')
    s = s.replace(u'»', ' ')
    s = s.replace(u'!', ' . ')
...
def gengram_sentence(corpus, N=4, sentence_count=20, start_seq="#"):
    ...
    # while (last_word != '#' or rand_text == '#') and last_word != '$':
        last_word = next_word(rand_text, min(len(rand_text.split()), N), counts)
...
def normalize_sentence_length(text):
    ...
    for x in text:
        ...
        new_text.append(' '.join(x.split(' ')) + ' . ')
    return ' '.join(new_text)

if __name__ == "__main__":

    corpus = preprocess_corpus("chekhov_f.txt")
    # corpus = preprocess_corpus("Chekhov.txt")
    f = codecs.open(output_file, 'w', 'utf-8')
    f.write(normalize_sentence_length(gengram_sentence(corpus, start_seq="#")))
    # print(normalize_sentence_length(gengram_sentence(corpus, start_seq="#")))
    f.close() 
